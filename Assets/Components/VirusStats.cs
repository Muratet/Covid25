...
public class VirusStats : MonoBehaviour {
    ...
    [Tooltip("Rate of the infected population from which the pandemic resolves by itself without special measures")]
    public float populationRatioImmunity = 0.6f;

    // Taille de la fenêtre glissante pour stocker le nombre de personnes infectées
    [Tooltip("Sliding window size to store the number of infected people")]
    ...
    [Tooltip("Age from which we can start to have deaths")]
    ...
    [Tooltip("Control of the curvature of the exponential, 1 => classical exponential, <1 => faster acceleration, >1 => slower acceleration")]
    public int curveStrenght = 12;

    [Tooltip("Serious case rate")]
    public float seriousRatio = 0.138f; // 13.8% des car sont sérieux
    [Tooltip("Critical case rate")]
